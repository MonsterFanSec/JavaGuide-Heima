...
public class GameJFrame extends JFrame implements KeyListener, ActionListener {
	...
	//定义一个变量，记录当前展示图片的路径
	String path = "JavaBase1/src/day30-阶段综合案例（带权重的随机&每日一记）/src/puzzlegame/image/animal/animal3/";
	...
	private void initImage() {
		...
		if (victory()) {
			//显示胜利的图标
			JLabel winJLabel = new JLabel(new ImageIcon("JavaBase1/src/day30-阶段综合案例（带权重的随机&每日一记）/src/puzzlegame/image/win.png"));
		...
		//添加背景图片
		JLabel background = new JLabel(new ImageIcon("JavaBase1/src/day30-阶段综合案例（带权重的随机&每日一记）/src/puzzlegame/image/background.png"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == replayItem) {
			...
			path = "JavaBase1/src/day30-阶段综合案例（带权重的随机&每日一记）/src/puzzlegame/image/girl/girl" + number + "/";
			//计步器清零
			step = 0;
			//再次打乱二维数组中的数据
			initData();
			...
			String str = item.getText();    // 按钮文本：存档1(空)
			int index = str.charAt(2) - '0';    // 按钮序号

			//直接把游戏的数据写到本地文件中
			try {
				ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("JavaBase1/src/day30-阶段综合案例（带权重的随机&每日一记）/src/puzzlegame/save/save" + index + ".data"));
			...
			try {
				//可以到本地文件中读取数据
				ObjectInputStream ois = new ObjectInputStream(new FileInputStream("JavaBase1/src/day30-阶段综合案例（带权重的随机&每日一记）/src/puzzlegame/save/save" + index + ".data"));
				gi = (GameInfo) ois.readObject();
				ois.close();
			} catch (IOException ioException) {
				ioException.printStackTrace();
