...
public class Test {
	public static void main(String[] args) throws IOException {
		// 1.读取正确的用户名和密码
		String path = "/Users/daykalif/Desktop/Java/JavaBase1/src/day30-阶段综合案例（带权重的随机&每日一记）/src/com/itheima/myiotest8";
		BufferedReader br = new BufferedReader(new FileReader(path + "/userinfo.txt"));
		...
		if (rightUsername.equals(username) && rightPassword.equals(password) && count < 3) {
			...
			writeInfo("username=" + rightUsername + "&password=" + rightPassword + "&count=0", path);
		} else {
			count++;
			if (count < 3) {
			...
			writeInfo("username=" + rightUsername + "&password=" + rightPassword + "&count=" + count, path);
		}
	}
	 ...
	 *       要写出的字符串
	 * */
	public static void writeInfo(String content, String path) throws IOException {
		BufferedWriter bw = new BufferedWriter(new FileWriter(path + "/userinfo.txt"));
		bw.write(content);
		bw.close();
	}
}