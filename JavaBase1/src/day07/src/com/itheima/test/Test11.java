...
public class Test11 {
	...
	public static int[] createNumber() {
		...
		for (int i = 0; i < 6; ) {
			//获取红球号码
			int redNumber = r.nextInt(33) + 1; // 红球范围：1-33
			boolean flag = contains(arr, redNumber);
			if (!flag) {
				//把红球号码添加到数组当中
				arr[i] = redNumber;
		...
		//生成蓝球号码并添加到数组当中
		int blueNumber = r.nextInt(16) + 1; // 蓝球范围：1-16
		arr[arr.length - 1] = blueNumber;
		return arr;
	}
	...
	//判断用户中奖情况
	public static void judgeNumber(int[] arr, int[] userInputArr) {
		//3.判断用户的中奖情况
		//红球 蓝球
		int redCount = 0;
		int blueCount = 0;
